...
ShopifyAPI.prototype.exchange_temporary_token = function(query_params, callback) {
  ...
  var non_state = !query_params['state'];
  if (!self.is_valid_signature(query_params, non_state)) {
...
ShopifyAPI.prototype.makeRequest = function(endpoint, method, data, callback, retry) {
  ...
  var request = https.request(options, function(response) {
    ...
    if (response.headers && response.headers.http_x_shopify_shop_api_call_limit) {
      self.conditional_console_log(
        'API_LIMIT: ' + response.headers.http_x_shopify_shop_api_call_limit
      );
    }
    ...
    response.on('data', function(chunk) {
      self.conditional_console_log('DOWNLOADING DATA');
      body += chunk;
    });
    ...
    response.on('end', function() {
      ...
      if (
        ...
        self.has_header(response, 'http_x_shopify_shop_api_call_limit')
      ) {
        var api_limit = parseInt(
          response.headers['http_x_shopify_shop_api_call_limit'].split('/')[0],
          10
        );
      ...
      setTimeout(function() {
        ...
        if (response.statusCode >= 400) {
          if (
            json &&
            (json.hasOwnProperty('error_description') ||
